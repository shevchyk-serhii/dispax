...
object AuthenticatedHandlersSpec extends ZIOSpecDefault {
  ...
  private def makeToken: ZIO[JwtService, Throwable, String] =
  ...
  private def requestWithToken(token: String): Request =
    Request.get(URL.decode("/test").toOption.get).addHeader("Authorization", s"Bearer $token")

  case class TestBody(name: String) derives JsonCodec

  def spec = suite("AuthenticatedHandlers")(

    suite("authenticatedHandler")(
      test("executes handler when token is valid") {
        val h: Handler[JwtService, Response, Request, Response] =
          AuthenticatedHandlers.authenticatedHandler { (user, _) =>
      ...
      test("returns 500 when handler throws unexpected exception") {
        val h: Handler[JwtService, Response, Request, Response] =
          AuthenticatedHandlers.authenticatedHandler { (_, _) =>
    ...
    suite("authenticatedPathHandler")(
      test("provides path param and user to handler") {
        val h: Handler[JwtService, Response, (String, Request), Response] =
          AuthenticatedHandlers.authenticatedPathHandler[Any, String] { (user, param, _) =>
