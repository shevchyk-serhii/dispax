...
object AuthApi:
  ...
  private val loginServer =
    loginEndpoint.zServerLogic[AuthEnv] { case (req, ip) =>
  ...
  private val validateServer: ZServerEndpoint[AuthEnv, Any] =
    validateEndpoint
  ...
  private val logoutServer =
    logoutEndpoint.zServerLogic[AuthEnv](_ => ZIO.succeed(AuthSuccessResponse(success = true)))

  private val passwordResetServer =
    passwordResetEndpoint.zServerLogic[AuthEnv](_ =>
      ZIO.succeed(AuthSuccessResponse(success = true, message = Some("Reset link sent if account exists")))
    )

  private val biometricSetupServer =
    biometricSetupEndpoint.zServerLogic[AuthEnv](_ =>
