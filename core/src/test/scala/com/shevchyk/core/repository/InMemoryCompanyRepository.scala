...
/**
 * Ref-backed in-memory implementation of [[CompanyRepository]] for use in unit tests.
 * Never used in integration tests — those use a real PostgreSQL via Testcontainers.
 */
class InMemoryCompanyRepository extends CompanyRepository:
  ...
  override def findAll(): Task[List[Company]] =
  ...
  override def create(company: Company): Task[Company] =
    store.update(_.updated(company.id, company)).as(company)
