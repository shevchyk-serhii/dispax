...
final case class UpdateNotificationPreferenceRequest(
  ...
  /**
   * Apply the patch onto existing preferences. Toggles default to their current value; the optional quiet-hours fields
   * keep the current value when unset (never cleared); `updatedAt` is refreshed.
   */
  def applyTo(current: NotificationPreference): NotificationPreference = current.copy(
