...
final case class UpdateCompanySettingsRequest(
  ...
  /** Apply the patch onto an existing settings object. Unset fields keep their
    * current value; `updatedAt` is refreshed.
    */
  def applyTo(current: CompanySettings): CompanySettings =
    current.copy(
