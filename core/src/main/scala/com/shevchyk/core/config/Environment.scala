...
object Environment {
   ...
   * `ConfigProvider.fromResourcePath()` reads) at `application-<env>.conf` unless an explicit `-Dconfig.resource=...`
   * was already supplied. Idempotent; safe to call before any config layer is built. Returns whether the property was
   * set by this call.
   */
