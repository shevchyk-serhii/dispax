...
- [x] **[HIGH] Мультироль игнорируется во всех Tapir secure-слоях, кроме `AppSecure`** _(проверено лично)_ — `ride/.../openapi/RideSecure.scala:38-61`, `driver/.../openapi/DriverApi.scala:75-99`, `schedule/.../openapi/ScheduleApi.scala:58-79` (`requireDispatcherOrAdmin`), `api/.../openapi/UserApi.scala:93-104` — строят `AuthenticatedUser` без `roles` и проверяют права только по primary-роли (`user.role`), хотя JWT несёт `roles` и эталон `api/.../openapi/AppSecure.scala:15-65` использует `effectiveRoles`. Заявлено «диспетчер может быть водителем» (`Person.roles: Set[PersonRole]`). Эффект: dispatcher-as-driver (primary=Dispatcher) получает 403 на driver-эндпоинтах поездок/расписания и не может вести свои поездки как водитель. Системная причина — копия `checkRole` без `effectiveRoles`. _Категория: Functional/Security._ — ИСПРАВЛЕНО (agent/fix-multirole-secure): общий `effectiveRoles`/`hasAnyRole` вынесены на `AuthenticatedUser` (auth-модуль, доступен всем слоям); RideSecure/DriverApi/ScheduleApi/UserApi теперь заполняют `roles` из JWT (`PersonRole.toWire`) и проверяют права через `hasAnyRole`; `AppSecure.effectiveRoles` делегирует туда же. `ScheduleApi.createUnavailability` получает driver-aware роль (`driverSelfRole`), чтобы dispatcher-as-driver мог отмечать свою недоступность.
...
- [ ] **[MEDIUM] SQL-метод PersonRepository.updateLastLogin без company_id** _(остаток defense-in-depth)_ — `core/.../repository/PostgresPersonRepository.scala` (`updateLastLogin`) — UPDATE без `AND company_id`. setAvatar/deleteAvatar уже закрыты (agent/fix-personrepo-tenant); updateLastLogin намеренно system-wide (вызывается при логине до знания компании). _Категория: Security._
