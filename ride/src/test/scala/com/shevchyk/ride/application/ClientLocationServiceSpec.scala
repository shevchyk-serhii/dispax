...
class InMemoryClientLocationRepository extends ClientLocationRepository:
  ...
  def getLocation(rideId: RideId): Task[Option[ClientLocation]] =
    ZIO.succeed(Option(store.get(rideId)))
...
class InMemoryDriverLocationProvider extends DriverLocationProvider:
  ...
  def getDriverLocation(driverId: PersonId): Task[Option[(Double, Double, Instant)]] =
    ZIO.succeed(locations.get(driverId))
...
object ClientLocationServiceSpec extends ZIOSpecDefault {
  ...
  def layers = InMemoryRideRepository.layer ++
  ...
  def createRide(withDriver: Boolean = true) =
    ZIO.serviceWithZIO[com.shevchyk.ride.repository.RideRepository](_.create(makeRide(withDriver)).orDie)

  def spec = suite("ClientLocationService")(
