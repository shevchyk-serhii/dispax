...
object ClientAddressServiceSpec extends ZIOSpecDefault {
  ...
  val testLayer =
    InMemoryClientAddressRepository.layer >>> ZLayer.fromFunction(ClientAddressServiceImpl(_))

  def req(address: String, label: String = "Home") =
    SaveClientAddressRequest(label = label, address = address)

  def spec = suite("ClientAddressService")(
    suite("getAddresses")(
