...
object ChatServiceSpec extends ZIOSpecDefault {
  ...
  def createRide(status: RideStatus) =
    ZIO.serviceWithZIO[com.shevchyk.ride.repository.RideRepository](_.create(makeRide(status)).orDie)

  def spec = suite("ChatService")(
    suite("sendMessage")(
