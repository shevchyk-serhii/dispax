...
object ClientAddressRoutesSpec extends ZIOSpecDefault {
  ...
  private def clientToken(userId: UUID = TestData.testUserId): ZIO[JwtService, Throwable, String] = TestJWT
    .generateToken(
      userId = userId,
  ...
  private val validAddressJson = """{"address":"Marienplatz 1, Munich","label":"Home"}"""

  private val clientIdStr = TestData.testUserId.toString

  def spec =
    suite("ClientAddressRoutes")(
