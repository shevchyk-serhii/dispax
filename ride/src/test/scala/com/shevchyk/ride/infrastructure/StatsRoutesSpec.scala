...
object StatsRoutesSpec extends ZIOSpecDefault {
  ...
  private def runRequest(request: Request): ZIO[RideService & ExpenseRepository & PersonRepository & RideRatingRepository & JwtService, Nothing, Response] =
    StatsRoutes.authenticatedRoutes.run(request).either.map {
      case Left(either) => either.merge
      case Right(response) => response
    }

  def spec = suite("StatsRoutes")(
    suite("GET /api/stats/payroll")(
      test("returns payroll summary for a driver") {
        for {
          ...
          request     = Request
                          .get(URL.decode(s"/api/stats/payroll?driverId=${TestData.testDriverId}&from=$from&to=$to").toOption.get)
