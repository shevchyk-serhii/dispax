...
/**
 * Integration tests for [[PostgresAirportConfigRepository]] against a real PostgreSQL database provided by
 * Testcontainers (reusable shared container, see [[PostgresTestContainer]]).
 *
...
object PostgresAirportConfigRepositorySpec extends ZIOSpecDefault {
  ...
  private def freshZone(airportCode: String, checkpointType: String): AirportCheckpointZone = AirportCheckpointZone(
  ...
  def spec =
    suite("PostgresAirportConfigRepository (real DB)")(
