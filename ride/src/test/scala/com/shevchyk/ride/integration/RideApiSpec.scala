...
object RideApiSpec extends ZIOSpecDefault {
  ...
  private def runRequest(
      request: Request
  ): ZIO[RideService & ClientAddressService & PersonRepository & JwtService, Nothing, Response] =
  ...
  def spec =
    suite("Ride API Integration Tests")(
      ...
      suite("GET /api/rides/driver/:id")(
        test("driver can fetch own rides") {
          for {
            ...
            ride1    = TestData
                         .createRide(clientId = PersonId(TestData.testUserId))
                         .copy(driverId = Some(PersonId(driverId)))
            ride2    = TestData
                         .createRide(clientId = PersonId(TestData.testUserId))
                         .copy(driverId = Some(PersonId(driverId)))
            _       <- repo.create(ride1)
