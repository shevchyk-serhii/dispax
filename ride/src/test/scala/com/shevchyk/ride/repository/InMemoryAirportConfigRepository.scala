 ...
 *
 * Follows the [[InMemoryRideRepository]] pattern: state is held in a [[Ref.Synchronized]] so tests can call methods
 * concurrently without race conditions. Used ONLY in unit tests — integration tests use the real PostgreSQL
 * implementation via Testcontainers (invariant §4).
 */
class InMemoryAirportConfigRepository extends AirportConfigRepository:

  private val state: Ref.Synchronized[Map[String, Airport]] = Unsafe.unsafe { implicit unsafe =>
    Runtime.default.unsafe.run(Ref.Synchronized.make(Map.empty[String, Airport])).getOrThrowFiberFailure()
  }

  override def findAll(): Task[List[Airport]] = state.get.map(_.values.toList)

  override def findByCode(code: String): Task[Option[Airport]] = state.get.map(_.get(code))

  override def create(airport: Airport): Task[Airport] = state.update(_.updated(airport.code, airport)).as(airport)

  override def update(code: String, airport: Airport): Task[Option[Airport]] = state.get.flatMap { m =>
  ...
  override def createZone(zone: AirportCheckpointZone): Task[AirportCheckpointZone] = state.get.flatMap { m =>
  ...
  override def updateZone(id: UUID, zone: AirportCheckpointZone): Task[Option[AirportCheckpointZone]] = state.get
    .flatMap { m =>
  ...
  override def deleteZone(id: UUID): Task[Boolean] = state.get.flatMap { m =>
    val airportOpt = m.values.find(_.zones.exists(_.id == id))
    airportOpt match
      case None          => ZIO.succeed(false)
      case Some(airport) =>
