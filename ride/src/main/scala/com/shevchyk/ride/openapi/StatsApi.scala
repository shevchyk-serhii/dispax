...
object StatsApi:
  ...
  private val payrollServer: ZServerEndpoint[StatsEnv, Any] =
    payrollEndpoint.serverLogic { user => (driverIdOpt, fromOpt, toOpt) =>
      (for {
  ...
  private val cancellationsServer: ZServerEndpoint[StatsEnv, Any] =
    cancellationsEndpoint.serverLogic { user => _ =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "ADMIN")
  ...
  private val peakHoursServer: ZServerEndpoint[StatsEnv, Any] =
    peakHoursEndpoint.serverLogic { user => daysOpt =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
  ...
  private val clientValueServer: ZServerEndpoint[StatsEnv, Any] =
    clientValueEndpoint.serverLogic { user => _ =>
      (for {
        _          <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId  <- requireCompanyId(user.companyId)
        service    <- ZIO.service[RideService]
  ...
  private val driverPerformanceServer: ZServerEndpoint[StatsEnv, Any] =
    driverPerformanceEndpoint.serverLogic { user => _ =>
      (for {
        _          <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId  <- requireCompanyId(user.companyId)
        service    <- ZIO.service[RideService]
  ...
  private val driverRatingsServer: ZServerEndpoint[StatsEnv, Any] =
    driverRatingsEndpoint.serverLogic { user => _ =>
      (for {
        _          <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId  <- requireCompanyId(user.companyId)
        personRepo <- ZIO.service[PersonRepository]
