...
object ClientAddressApi:
  ...
  private def parseClientPid(value: String): ZIO[Any, Err, PersonId] = ZIO
    .attempt(PersonId(java.util.UUID.fromString(value)))
    .mapError(_ => internalError)

  private def parseAddressId(value: String): ZIO[Any, Err, ClientAddressId] = ZIO
    .attempt(ClientAddressId(java.util.UUID.fromString(value)))
    .mapError(_ => internalError)

  // -- Endpoint descriptions -----------------------------------------------

  val getAddressesEndpoint = secureEndpoint.get
    .in("api" / "clients" / path[String]("clientId") / "addresses")
    .out(jsonBody[List[ClientAddress]])
    .tag(addressTag)
    .summary("List a client's saved addresses")

  val saveAddressEndpoint = secureEndpoint.post
    .in("api" / "clients" / path[String]("clientId") / "addresses")
  ...
  val updateAddressEndpoint = secureEndpoint.patch
  ...
  val deleteAddressEndpoint = secureEndpoint.delete
    .in("api" / "clients" / path[String]("clientId") / "addresses" / path[String]("addressId"))
  ...
  private val getAddressesServer: ZServerEndpoint[ClientAddressEnv, Any] = getAddressesEndpoint.serverLogic {
    user => clientId =>
      for {
  ...
  private val saveAddressServer: ZServerEndpoint[ClientAddressEnv, Any] = saveAddressEndpoint.serverLogic {
    user => (clientId, req) =>
      for {
        pid     <- parseClientPid(clientId)
  ...
  private val updateAddressServer: ZServerEndpoint[ClientAddressEnv, Any] = updateAddressEndpoint.serverLogic {
    user => (clientId, addressId, req) =>
      for {
        pid     <- parseClientPid(clientId)
        _       <- checkRoleOrOwner(user, pid.value, "DISPATCHER", "SECRETARY")
  ...
  private val deleteAddressServer: ZServerEndpoint[ClientAddressEnv, Any] = deleteAddressEndpoint.serverLogic {
    user => (clientId, addressId) =>
      for {
        pid     <- parseClientPid(clientId)
        _       <- checkRoleOrOwner(user, pid.value, "DISPATCHER", "SECRETARY")
        aid     <- parseAddressId(addressId)
  ...
  val serverEndpoints: List[ZServerEndpoint[ClientAddressEnv, Any]] = List(
    getAddressesServer,
    saveAddressServer,
    updateAddressServer,
    deleteAddressServer
