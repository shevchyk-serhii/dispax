...
object InvoiceApi:
  ...
  private val listInvoicesServer: ZServerEndpoint[BillingEnv, Any] =
    listInvoicesEndpoint.serverLogic { user => (statusOpt, limitOpt, offsetOpt) =>
      (for {
  ...
  private val createInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    createInvoiceEndpoint.serverLogic { user => req =>
      (for {
        companyId <- requireCompanyId(user.companyId)
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        service   <- ZIO.service[InvoiceService]
  ...
  private val getInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    getInvoiceEndpoint.serverLogic { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
  ...
  private val autoFillInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    autoFillInvoiceEndpoint.serverLogic { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
  ...
  private val fillFromRidesServer: ZServerEndpoint[BillingEnv, Any] =
    fillFromRidesEndpoint.serverLogic { user => (id, req) =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
  ...
  private val billableRidesServer: ZServerEndpoint[BillingEnv, Any] =
    billableRidesEndpoint.serverLogic { user => (clientCompanyIdStr, fromOpt, toOpt) =>
      (for {
        companyId       <- requireCompanyId(user.companyId)
        _               <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
  ...
  private val rideReceiptServer: ZServerEndpoint[BillingEnv, Any] =
    rideReceiptEndpoint.serverLogic { user => (rideId, taxRateOpt) =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
  ...
  private val invoicePdfServer: ZServerEndpoint[BillingEnv, Any] =
    invoicePdfEndpoint.serverLogic { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
  ...
  private val sendInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    sendInvoiceEndpoint.serverLogic { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
  ...
  private val payInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    payInvoiceEndpoint.serverLogic { user => (id, req) =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
  ...
  private val deleteInvoiceServer: ZServerEndpoint[BillingEnv, Any] =
    deleteInvoiceEndpoint.serverLogic { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "SECRETARY", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        invoiceId <- parseInvoiceId(id)
