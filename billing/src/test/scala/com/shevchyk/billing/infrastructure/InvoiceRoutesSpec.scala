...
object InvoiceRoutesSpec extends ZIOSpecDefault {
  ...
  private val inMemoryInvoiceRepo: ZLayer[Any, Nothing, InvoiceRepository] = ZLayer.succeed {
    new InvoiceRepository {
      ...
      def findByCompany(taxiCompanyId: CompanyId, status: Option[InvoiceStatus], limit: Int, offset: Int): Task[List[Invoice]] =
        ZIO.succeed(store.values().asScala.filter(_.taxiCompanyId == taxiCompanyId).toList.drop(offset).take(limit))
      ...
      def addItems(items: List[InvoiceItem]): Task[Unit] = ZIO.succeed {
        items.groupBy(_.invoiceId).foreach { case (iid, is) =>
          itemsStore.merge(iid, is, _ ++ _)
        }
      }
  ...
  private val testLayers =
    (inMemoryInvoiceRepo ++ inMemoryClientCompanyRepo >>> InvoiceService.layer) ++ testJwtService
