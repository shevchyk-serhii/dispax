...
final case class UpdateScheduleDayRequest(
  ...
  /** Apply the patch onto an existing schedule day. The `startTime`, `endTime`
    * and `status` values are passed in already validated by the caller (time
    * range and status-transition checks happen there); `notes` is merged from
    * this request and `updatedAt` is refreshed.
    */
  def applyTo(
      current: ScheduleDay,
      startTime: LocalTime,
