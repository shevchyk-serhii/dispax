...
object ScheduleApi:
  ...
  private def toError(ex: Throwable): (StatusCode, ApiError) =
    ex match
      ...
      case ScheduleError.DriverNotFound(id)                   =>
      ...
      case ScheduleError.DatabaseError(_)                     =>
  ...
  private def requireCompanyId(user: AuthenticatedUser): ZIO[Any, ScheduleError, CompanyId] =
    ZIO
  ...
  val createScheduleBatchEndpoint =
    secureEndpoint.post
  ...
  val getScheduleForDateEndpoint =
    secureEndpoint.get
  ...
  private val createScheduleDayServer: ZServerEndpoint[ScheduleEnv, Any] =
    createScheduleDayEndpoint.serverLogic { user => apiRequest =>
      (for {
  ...
  private val createScheduleBatchServer: ZServerEndpoint[ScheduleEnv, Any] =
    createScheduleBatchEndpoint.serverLogic { user => apiRequest =>
      (for {
        companyId     <- requireCompanyId(user)
        validRequest  <- apiRequest.validate
  ...
  private val getDriverScheduleServer: ZServerEndpoint[ScheduleEnv, Any] =
    getDriverScheduleEndpoint.serverLogic { user => driverId =>
      (for {
        companyId <- requireCompanyId(user)
        driverPid <- ZIO
  ...
  private val getScheduleForDateServer: ZServerEndpoint[ScheduleEnv, Any] =
    getScheduleForDateEndpoint.serverLogic { user => date =>
      (for {
        companyId <- requireCompanyId(user)
        localDate <- ZIO
  ...
  private val getScheduleRangeServer: ZServerEndpoint[ScheduleEnv, Any] =
    getScheduleRangeEndpoint.serverLogic { user => (fromOpt, toOpt) =>
      (for {
        companyId <- requireCompanyId(user)
        fromParam <- ZIO
                       .fromOption(fromOpt)
  ...
  private val updateScheduleDayServer: ZServerEndpoint[ScheduleEnv, Any] =
    updateScheduleDayEndpoint.serverLogic { user => (id, apiRequest) =>
      (for {
        companyId <- requireCompanyId(user)
  ...
  private val deleteScheduleDayServer: ZServerEndpoint[ScheduleEnv, Any] =
    deleteScheduleDayEndpoint.serverLogic { user => id =>
      (for {
        companyId <- requireCompanyId(user)
        schedId   <- ZIO
                       .attempt(ScheduleDayId(UUID.fromString(id)))
