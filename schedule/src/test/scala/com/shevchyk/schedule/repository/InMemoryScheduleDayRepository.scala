...
class InMemoryScheduleDayRepository extends ScheduleDayRepository:
  ...
  override def create(scheduleDay: ScheduleDay): Task[ScheduleDay] =
    store.get.flatMap { current =>
      val duplicate = current.values.exists(d =>
  ...
  override def findByDriverId(driverId: PersonId): Task[List[ScheduleDay]] =
    store.get.map(_.values.filter(_.driverId == driverId).toList.sortBy(_.date))

  override def findByDriverAndDate(driverId: PersonId, date: LocalDate): Task[Option[ScheduleDay]] =
    store.get.map(_.values.find(d => d.driverId == driverId && d.date == date))

  override def findByCompanyAndDate(companyId: CompanyId, date: LocalDate): Task[List[ScheduleDay]] =
    store.get.map(_.values.filter(d => d.companyId == companyId && d.date == date).toList.sortBy(_.startTime))

  override def findByCompanyAndDateRange(companyId: CompanyId, from: LocalDate, to: LocalDate): Task[List[ScheduleDay]] =
    store.get.map(
      _.values
  ...
  override def delete(id: ScheduleDayId): Task[Unit] =
    store.update(_.removed(id)).unit

object InMemoryScheduleDayRepository:
  val layer: ZLayer[Any, Nothing, ScheduleDayRepository] =
    ZLayer.succeed(new InMemoryScheduleDayRepository)
