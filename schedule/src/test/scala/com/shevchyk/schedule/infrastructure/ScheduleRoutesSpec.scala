...
object ScheduleRoutesSpec extends ZIOSpecDefault {
  ...
  private val testLayers = ((InMemoryScheduleDayRepository.layer ++ noopPersonRepo) >>> ScheduleService.layer) ++ testJwtService
  ...
  def spec = suite("ScheduleRoutes")(
    ...
    suite("POST /api/schedules")(
      test("dispatcher can create schedule day (201, 400, or 404)") {
        ...
        } yield assertTrue(response.status == Status.Created || response.status == Status.BadRequest || response.status == Status.NotFound)
      }.provide(testLayers),
    ...
    suite("GET /api/schedules (with date range params)")(
      test("dispatcher can list schedules with date params (200)") {
        for {
          tok      <- token(PersonRole.Dispatcher, dispatcherId)
          response <- run(Request.get(URL.decode("/api/schedules?from=2026-08-01&to=2026-08-31").toOption.get)
                        .addHeader(Header.Authorization.Bearer(tok)))
      ...
      test("dispatcher gets 404 or 403 for non-existent schedule") {
        for {
          ...
          response <- run(Request.delete(URL.decode(s"/api/schedules/${UUID.randomUUID()}").toOption.get)
                        .addHeader(Header.Authorization.Bearer(tok)))
        } yield assertTrue(response.status == Status.NotFound || response.status == Status.Forbidden || response.status == Status.InternalServerError)
      }.provide(testLayers)
    ),
