...
**API docs (Swagger UI):** `/docs` — OpenAPI document at `/docs/docs.yaml` (YAML only; no JSON variant). Local: `http://localhost:8080/docs`, prod: `https://dispax-o2trzxjbva-ew.a.run.app/docs`. Generated via Tapir `SwaggerInterpreter` in `api/.../app/openapi/OpenApiServer.scala` — single source of truth for all 147 endpoints.  
