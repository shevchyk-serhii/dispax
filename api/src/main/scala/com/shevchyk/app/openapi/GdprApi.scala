 ...
 * `GdprRoutes` while preserving paths, request/response shapes, role checks, company isolation, status codes and
 * error mapping.
 */
object GdprApi:
  ...
  private val getConsentsServer: ZServerEndpoint[GdprEnv, Any] =
    getConsentsEndpoint.serverLogic[GdprEnv] { user => _ =>
      (for {
  ...
  private val updateConsentServer: ZServerEndpoint[GdprEnv, Any] =
    updateConsentEndpoint.serverLogic[GdprEnv] { user => req =>
      (for {
        repo   <- ZIO.service[GdprRepository]
  ...
  private val exportServer: ZServerEndpoint[GdprEnv, Any] =
    exportEndpoint.serverLogic[GdprEnv] { user => _ =>
      (for {
  ...
  private val deletionRequestServer: ZServerEndpoint[GdprEnv, Any] =
    deletionRequestEndpoint.serverLogic[GdprEnv] { user => _ =>
      (for {
        repo    <- ZIO.service[GdprRepository]
        req      = GdprRequest(
  ...
  private val requestsServer: ZServerEndpoint[GdprEnv, Any] =
    requestsEndpoint.serverLogic[GdprEnv] { user => _ =>
      (for {
