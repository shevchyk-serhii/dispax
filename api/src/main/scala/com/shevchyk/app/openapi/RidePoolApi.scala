...
object RidePoolApi:
  ...
  private val createServer: ZServerEndpoint[RidePoolEnv, Any] =
    createEndpoint.serverLogic[RidePoolEnv] { user => req =>
      (for {
        ...
        _         <- ZIO.foreach(req.rideIds.zipWithIndex) { case (rideIdStr, idx) =>
                       for {
        ...
        updated   <- if req.rideIds.nonEmpty then
                       repo.update(created.copy(currentPassengers = req.rideIds.size)).mapError(internal)
  ...
  private val listServer: ZServerEndpoint[RidePoolEnv, Any] =
    listEndpoint.serverLogic[RidePoolEnv] { user => _ =>
      (for {
        _         <- checkRole(user, "DISPATCHER")
        repo      <- ZIO.service[RidePoolRepository]
        companyId <- requireCompanyId(user.companyId)
  ...
  private val listOpenServer: ZServerEndpoint[RidePoolEnv, Any] =
    listOpenEndpoint.serverLogic[RidePoolEnv] { user => _ =>
      (for {
        _         <- checkRole(user, "DISPATCHER")
        repo      <- ZIO.service[RidePoolRepository]
        companyId <- requireCompanyId(user.companyId)
  ...
  private val getServer: ZServerEndpoint[RidePoolEnv, Any] =
    getEndpoint.serverLogic[RidePoolEnv] { user => id =>
      (for {
  ...
  private val addRideServer: ZServerEndpoint[RidePoolEnv, Any] =
    addRideEndpoint.serverLogic[RidePoolEnv] { user =>
      { case (id, req) =>
        (for {
          _            <- checkRole(user, "DISPATCHER")
          companyId    <- requireCompanyId(user.companyId)
  ...
  private val removeRideServer: ZServerEndpoint[RidePoolEnv, Any] =
    removeRideEndpoint.serverLogic[RidePoolEnv] { user =>
      { case (id, rideId) =>
        (for {
          _            <- checkRole(user, "DISPATCHER")
          companyId    <- requireCompanyId(user.companyId)
  ...
  private val assignServer: ZServerEndpoint[RidePoolEnv, Any] =
    assignEndpoint.serverLogic[RidePoolEnv] { user =>
      { case (id, driverReq) =>
        (for {
          _           <- checkRole(user, "DISPATCHER")
          companyId   <- requireCompanyId(user.companyId)
  ...
  private val statusServer: ZServerEndpoint[RidePoolEnv, Any] =
    statusEndpoint.serverLogic[RidePoolEnv] { user =>
      { case (id, statusReq) =>
        (for {
          _          <- checkRole(user, "DISPATCHER", "DRIVER")
          companyId  <- requireCompanyId(user.companyId)
  ...
  private val findByRideServer: ZServerEndpoint[RidePoolEnv, Any] =
    findByRideEndpoint.serverLogic[RidePoolEnv] { user => rideId =>
      (for {
        companyId    <- requireCompanyId(user.companyId)
        repo         <- ZIO.service[RidePoolRepository]
        parsedRideId <- parseRideId(rideId)
