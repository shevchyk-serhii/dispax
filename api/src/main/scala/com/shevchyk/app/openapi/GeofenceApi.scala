...
object GeofenceApi:
  ...
  val endpoints = List(createEndpoint, alertsEndpoint, alertsByDriverEndpoint, listEndpoint, updateEndpoint, deleteEndpoint)

  // -- Server logic ---------------------------------------------------------

  private val createServer: ZServerEndpoint[GeofenceEnv, Any] =
    createEndpoint.serverLogic[GeofenceEnv] { user => req =>
      (for {
  ...
  private val listServer: ZServerEndpoint[GeofenceEnv, Any] =
    listEndpoint.serverLogic[GeofenceEnv] { user => _ =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        repo      <- ZIO.service[GeofenceRepository]
  ...
  private val updateServer: ZServerEndpoint[GeofenceEnv, Any] =
    updateEndpoint.serverLogic[GeofenceEnv] { user =>
      { case (id, req) =>
        (for {
          _         <- checkRole(user, "DISPATCHER", "ADMIN")
          companyId <- requireCompanyId(user.companyId)
  ...
  private val deleteServer: ZServerEndpoint[GeofenceEnv, Any] =
    deleteEndpoint.serverLogic[GeofenceEnv] { user => id =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
        geoId     <- parseUuid(id).map(GeofenceId(_))
  ...
  private val alertsServer: ZServerEndpoint[GeofenceEnv, Any] =
    alertsEndpoint.serverLogic[GeofenceEnv] { user => limitOpt =>
      (for {
        _         <- checkRole(user, "DISPATCHER", "ADMIN")
        companyId <- requireCompanyId(user.companyId)
  ...
  private val alertsByDriverServer: ZServerEndpoint[GeofenceEnv, Any] =
    alertsByDriverEndpoint.serverLogic[GeofenceEnv] { user =>
