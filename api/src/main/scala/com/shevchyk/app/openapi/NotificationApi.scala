...
object NotificationApi:
  ...
  private val listServer: ZServerEndpoint[NotificationEnv, Any] =
    listEndpoint.serverLogic[NotificationEnv] { user =>
  ...
  private val unreadCountServer: ZServerEndpoint[NotificationEnv, Any] =
    unreadCountEndpoint.serverLogic[NotificationEnv] { user => _ =>
      (for {
        repo  <- ZIO.service[NotificationRepository]
  ...
  private val markAsReadServer: ZServerEndpoint[NotificationEnv, Any] =
    markAsReadEndpoint.serverLogic[NotificationEnv] { user => id =>
      (for {
        repo   <- ZIO.service[NotificationRepository]
  ...
  private val markAllReadServer: ZServerEndpoint[NotificationEnv, Any] =
    markAllReadEndpoint.serverLogic[NotificationEnv] { user => _ =>
      (for {
        repo <- ZIO.service[NotificationRepository]
  ...
  private val deleteServer: ZServerEndpoint[NotificationEnv, Any] =
    deleteEndpoint.serverLogic[NotificationEnv] { user => id =>
      (for {
        repo    <- ZIO.service[NotificationRepository]
        nId     <- parseUuid(id).map(AppNotificationId(_))
  ...
  private val deleteAllServer: ZServerEndpoint[NotificationEnv, Any] =
    deleteAllEndpoint.serverLogic[NotificationEnv] { user => _ =>
      (for {
        repo <- ZIO.service[NotificationRepository]
