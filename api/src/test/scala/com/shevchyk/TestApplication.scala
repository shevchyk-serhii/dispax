...
import com.shevchyk.core.domain.{RidePool, RidePoolId, RidePoolMember, RidePoolMemberId, PoolStatus, PoolMemberStatus, Session, SessionId}
...
import com.shevchyk.ride.domain.{ClientAddress, ClientAddressId, ClientLocation, DriverEarnings, Expense, ExpenseCategory, ExpenseId, RecurrencePattern, Ride, RideStatus, RideTemplate, RideTemplateId}
...
object TestApplication extends ZIOAppDefault:
    ...
    new PersonRepository {
      private val people = Map[PersonId, Person](
        testPersonId1 -> Person(
          testPersonId1, "Test User", "test@example.com", PersonRole.Client,
          passwordHash = hashPassword("Password123"), phone = Some("+1234567890"),
          companyId = Some(testCompanyId1)
        ),
        testPersonId50 -> Person(
          testPersonId50, "Client User", "client@example.com", PersonRole.Client,
          passwordHash = hashPassword("Password123"), phone = Some("+1111111111"),
          companyId = Some(testCompanyId1)
        ),
        testPersonId10 -> Person(
          testPersonId10, "Driver User", "driver@example.com", PersonRole.Driver,
          passwordHash = hashPassword("Password123"), phone = Some("+2222222222"),
          companyId = Some(testCompanyId1)
        ),
        testPersonId99 -> Person(
          testPersonId99, "Admin User", "admin@example.com", PersonRole.Admin,
          passwordHash = hashPassword("Password123"), phone = Some("+3333333333"),
          companyId = Some(testCompanyId1)
        ),
        testPersonId33 -> Person(
          testPersonId33, "Dispatcher User", "dispatcher@example.com", PersonRole.Dispatcher,
          passwordHash = hashPassword("Password123"), phone = Some("+4444444444"),
          companyId = Some(testCompanyId1)
        ),
        testPersonId44 -> Person(
          testPersonId44, "Secretary User", "secretary@example.com", PersonRole.Secretary,
          passwordHash = hashPassword("Password123"), phone = Some("+5555555555"),
          companyId = Some(testCompanyId1)
        )
      )
      ...
      def findByRoleAndCompany(role: PersonRole, companyId: CompanyId): Task[List[Person]]          = ZIO.succeed(people.values.filter(p => p.role == role && p.companyId.contains(companyId)).toList)
      def findByCompanyId(companyId: CompanyId): Task[List[Person]]                                 = ZIO.succeed(people.values.filter(_.companyId.contains(companyId)).toList)
      ...
      def searchByQuery(query: String): Task[List[Person]]                                          = ZIO.succeed(people.values.filter(p => p.name.toLowerCase.contains(query.toLowerCase) || p.email.toLowerCase.contains(query.toLowerCase)).toList)
  ...
  private val testStaticTokenPayloads: Map[String, JwtPayload] = {
    ...
    Map(
      "valid-token-1"  -> JwtPayload(testPersonId1.value,  "test@example.com",       PersonRole.Client,     Some(testCompanyId1.value), None, iat, exp),
      "valid-token-50" -> JwtPayload(testPersonId50.value, "client@example.com",     PersonRole.Client,     Some(testCompanyId1.value), None, iat, exp),
      "valid-token-10" -> JwtPayload(testPersonId10.value, "driver@example.com",     PersonRole.Driver,     Some(testCompanyId1.value), None, iat, exp),
      "valid-token-99" -> JwtPayload(testPersonId99.value, "admin@example.com",      PersonRole.Admin,      Some(testCompanyId1.value), None, iat, exp),
      "valid-token-33" -> JwtPayload(testPersonId33.value, "dispatcher@example.com", PersonRole.Dispatcher, Some(testCompanyId1.value), None, iat, exp),
      "valid-token-44" -> JwtPayload(testPersonId44.value, "secretary@example.com",  PersonRole.Secretary,  Some(testCompanyId1.value), None, iat, exp)
    )
  }

  private val testJwtServiceLayer: ZLayer[JwtConfig, Nothing, JwtService] =
    ZLayer.fromZIO {
  ...
  private val inMemoryRideRepositoryLayer: ZLayer[Any, Nothing, RideRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(Map[RideId, Ride](
        ...
        new RideRepository:
          ...
          def findByClientId(cid: PersonId): Task[List[Ride]]   = ridesRef.get.map(_.values.filter(_.clientId == cid).toList)
          def findByDriverId(did: PersonId): Task[List[Ride]]   = ridesRef.get.map(_.values.filter(_.driverId.contains(did)).toList)
          def findByStatus(s: RideStatus): Task[List[Ride]]     = ridesRef.get.map(_.values.filter(_.status == s).toList)
          def findByCompanyId(cid: CompanyId): Task[List[Ride]] = ridesRef.get.map(_.values.filter(_.companyId == cid).toList)
          ...
          def countByCompanyGroupedByStatus(cid: CompanyId): Task[Map[String, Int]] =
            ridesRef.get.map(_.values.filter(_.companyId == cid).groupBy(_.status.toString).map((k, v) => k -> v.size))
          def sumRevenueByCompany(cid: CompanyId): Task[BigDecimal] =
            ridesRef.get.map(_.values.filter(r => r.companyId == cid && r.status == RideStatus.Completed).flatMap(r => r.finalPrice.orElse(r.estimatedPrice)).sum)
          def sumTodayRevenueByCompany(cid: CompanyId): Task[BigDecimal] =
            ...
            ridesRef.get.map(_.values.filter(r => r.companyId == cid && r.status == RideStatus.Completed && r.endTime.exists(!_.isBefore(start))).flatMap(r => r.finalPrice.orElse(r.estimatedPrice)).sum)
          def avgAssignmentMinutesByCompany(cid: CompanyId): Task[Double] =
            ridesRef.get.map { all =>
              ...
              else assigned.map(r => java.time.Duration.between(r.requestTime, r.startTime.get).toMinutes.toDouble).sum / assigned.size
            }
          def countDailyStatsByCompany(cid: CompanyId, days: Int): Task[List[(String, Int, Int, Int)]] =
            ...
            ridesRef.get.map(_.values.filter(r => r.companyId == cid && r.requestTime.isAfter(cutoff)).groupBy(r => r.requestTime.atZone(ZoneOffset.UTC).toLocalDate.toString).map { case (d, rs) => (d, rs.size, rs.count(_.status == RideStatus.Completed), rs.count(_.status == RideStatus.Cancelled)) }.toList.sortBy(_._1))
          def earningsByDriver(did: PersonId, cid: CompanyId, from: Instant, to: Instant): Task[DriverEarnings] =
            ridesRef.get.map { all =>
              val inScope = all.values.filter(r => r.driverId.contains(did) && r.companyId == cid && !periodTime(r).isBefore(from) && periodTime(r).isBefore(to))
              DriverEarnings(inScope.filter(_.status == RideStatus.Completed).flatMap(r => r.finalPrice.orElse(r.estimatedPrice)).sum, inScope.count(_.status == RideStatus.Completed), inScope.count(_.status == RideStatus.Cancelled))
            }
          def earningsBucketsByDriver(did: PersonId, cid: CompanyId, from: Instant, to: Instant, bucket: TimeBucket): Task[List[(Instant, BigDecimal)]] =
            ridesRef.get.map { all =>
              ...
              all.values.filter(r => r.driverId.contains(did) && r.companyId == cid && r.status == RideStatus.Completed && !periodTime(r).isBefore(from) && periodTime(r).isBefore(to))
                .groupBy(r => trunc(periodTime(r))).view.mapValues(_.flatMap(r => r.finalPrice.orElse(r.estimatedPrice)).sum).toList.sortBy(_._1)
            }
          def findAssignedRidesInWindow(from: Instant, to: Instant): Task[List[Ride]] =
            ridesRef.get.map(_.values.filter(r => r.status == RideStatus.Assigned && r.scheduledTime.exists(t => !t.isBefore(from) && !t.isAfter(to))).toList)
  ...
  private val inMemoryClientAddressRepositoryLayer: ZLayer[Any, Nothing, ClientAddressRepository] =
    ZLayer.succeed {
      new ClientAddressRepository:
        private val store = new ConcurrentHashMap[ClientAddressId, ClientAddress](Map(testClientAddressId -> testClientAddress).asJava)
        def findByClient(clientId: PersonId): Task[List[ClientAddress]]                                                               = ZIO.succeed(store.values.asScala.filter(_.clientId == clientId).toList)
        def save(address: ClientAddress): Task[ClientAddress]                                                                          = ZIO.succeed { store.put(address.id, address); address }
        def incrementUseCount(id: ClientAddressId): Task[Unit]                                                                        = ZIO.succeed(Option(store.get(id)).foreach(a => store.put(id, a.copy(useCount = a.useCount + 1))))
        def delete(id: ClientAddressId, clientId: PersonId): Task[Boolean]                                                            = ZIO.succeed(Option(store.get(id)) match { case Some(a) if a.clientId == clientId => store.remove(id); true; case _ => false })
        def findByAddressText(clientId: PersonId, address: String): Task[Option[ClientAddress]]                                        = ZIO.succeed(store.values.asScala.find(a => a.clientId == clientId && a.address == address))
        def updateLabelAndAliases(id: ClientAddressId, clientId: PersonId, label: Option[String], aliases: Option[List[String]]): Task[Option[ClientAddress]] =
          ZIO.succeed(Option(store.get(id)).filter(_.clientId == clientId).map { a => val u = a.copy(label = label.getOrElse(a.label), aliases = aliases.getOrElse(a.aliases)); store.put(id, u); u })
    }

  private val inMemoryScheduleDayRepositoryLayer: ZLayer[Any, Nothing, ScheduleDayRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(Map.empty[ScheduleDayId, ScheduleDay]).map { store =>
        new ScheduleDayRepository:
          ...
          def findByDriverId(driverId: PersonId): Task[List[ScheduleDay]]                                      = store.get.map(_.values.filter(_.driverId == driverId).toList.sortBy(_.date))
          def findByDriverAndDate(driverId: PersonId, date: LocalDate): Task[Option[ScheduleDay]]              = store.get.map(_.values.find(d => d.driverId == driverId && d.date == date))
          def findByCompanyAndDate(companyId: CompanyId, date: LocalDate): Task[List[ScheduleDay]]             = store.get.map(_.values.filter(d => d.companyId == companyId && d.date == date).toList.sortBy(_.startTime))
          def findByCompanyAndDateRange(companyId: CompanyId, from: LocalDate, to: LocalDate): Task[List[ScheduleDay]] =
            store.get.map(_.values.filter(d => d.companyId == companyId && !d.date.isBefore(from) && !d.date.isAfter(to)).toList.sortBy(d => (d.date, d.startTime)))
          def update(scheduleDay: ScheduleDay): Task[ScheduleDay]                                              = store.update(_.updated(scheduleDay.id, scheduleDay)).as(scheduleDay)
  ...
  private val inMemoryBillingClientCompanyRepositoryLayer: ZLayer[Any, Nothing, BillingClientCompanyRepository] =
    ZLayer.succeed {
      new BillingClientCompanyRepository:
        ...
        def create(req: CreateClientCompanyRequest, taxiCompanyId: CompanyId): Task[ClientCompany] =
          val cc = ClientCompany(ClientCompanyId(UUID.randomUUID()), req.name, taxiCompanyId, req.email, req.phone, req.address)
          ZIO.succeed { store.put(cc.id, cc); cc }
        def update(id: ClientCompanyId, req: CreateClientCompanyRequest): Task[Option[ClientCompany]] =
          ZIO.succeed(Option(store.get(id)).map { old =>
        ...
        def delete(id: ClientCompanyId): Task[Boolean] =
          ZIO.succeed(Option(store.get(id)).isDefined && { store.remove(id); true })
    }
  ...
  private val inMemoryInvoiceRepositoryLayer: ZLayer[Any, Nothing, InvoiceRepository] =
    ZLayer.succeed {
      new InvoiceRepository:
        ...
        def nextInvoiceNumber(taxiCompanyId: CompanyId, year: Int): Task[String]                                    = ZIO.succeed(s"INV-$year-${UUID.randomUUID().toString.take(8)}")
        ...
        def findByCompany(taxiCompanyId: CompanyId, status: Option[com.shevchyk.billing.domain.InvoiceStatus], limit: Int, offset: Int): Task[List[Invoice]] =
          ZIO.succeed(store.values.asScala.filter(_.taxiCompanyId == taxiCompanyId).drop(offset).take(limit).toList)
        def update(invoice: Invoice): Task[Invoice]                                                                   = ZIO.succeed { store.put(invoice.id, invoice); invoice }
        def delete(id: InvoiceId): Task[Boolean]                                                                      = ZIO.succeed(Option(store.get(id)).isDefined && { store.remove(id); true })
        def addItems(items: List[InvoiceItem]): Task[Unit]                                                            = ZIO.succeed(items.groupBy(_.invoiceId).foreach((k, v) => itemsStore.put(k, v)))
        def deleteItems(invoiceId: InvoiceId): Task[Unit]                                                             = ZIO.succeed(itemsStore.remove(invoiceId))
        def findUnbilledRides(clientCompanyId: ClientCompanyId, from: java.time.LocalDate, to: java.time.LocalDate): Task[List[UnbilledRide]] =
          ZIO.succeed(Nil)
    }

  private val inMemoryDriverLocationRepositoryLayer: ZLayer[Any, Nothing, DriverLocationRepository] =
    ZLayer.succeed {
      new DriverLocationRepository:
        ...
        def updateLocation(driverId: PersonId, latitude: Double, longitude: Double): Task[Unit] =
          ZIO.succeed(locations.put(driverId, DriverLocation(driverId, latitude, longitude)))
        def getLocation(driverId: PersonId): Task[Option[DriverLocation]]  = ZIO.succeed(Option(locations.get(driverId)))
        def updateAvailability(driverId: PersonId, status: String): Task[Unit] = ZIO.succeed(availability.put(driverId, status))
        ...
        def findAvailableByCompanyId(companyId: CompanyId): Task[List[(PersonId, String, Option[Double], Option[Double])]] =
          ZIO.succeed(availability.asScala.map { case (id, s) => val l = Option(locations.get(id)); (id, s, l.map(_.latitude), l.map(_.longitude)) }.toList)
    }

  private val noopHereRoutingServiceLayer: ZLayer[Any, Nothing, HereRoutingService] =
    ZLayer.succeed(new HereRoutingService:
      def getEtaMinutes(originLat: Double, originLng: Double, destLat: Double, destLng: Double): Task[Option[Int]] =
        ZIO.succeed(Some(10))
    )

  private val inMemoryClientLocationRepositoryLayer: ZLayer[Any, Nothing, ClientLocationRepository] =
    ZLayer.succeed {
      new ClientLocationRepository:
        ...
        def updateLocation(rideId: RideId, clientId: PersonId, latitude: Double, longitude: Double): Task[Unit] =
          ZIO.succeed(store.put(rideId, ClientLocation(rideId, clientId, latitude, longitude)))
  ...
  private val inMemoryRidePoolRepositoryLayer: ZLayer[Any, Nothing, RidePoolRepository] =
    ZLayer.fromZIO(
      for {
        ...
        def findByCompanyId(companyId: CompanyId): Task[List[RidePool]] = poolsRef.get.map(_.values.filter(_.companyId == companyId).toList.sortBy(_.createdAt).reverse)
        def findOpenPools(companyId: CompanyId): Task[List[RidePool]] = poolsRef.get.map(_.values.filter(p => p.companyId == companyId && p.status == PoolStatus.Open).toList)
        ...
        def findMembersByPoolId(poolId: RidePoolId): Task[List[RidePoolMember]] = membersRef.get.map(_.filter(_.poolId == poolId).sortBy(_.pickupOrder))
        def findPoolByRideId(rideId: RideId): Task[Option[RidePool]] = membersRef.get.flatMap(ms => poolsRef.get.map(ps => ms.find(_.rideId == rideId).flatMap(m => ps.get(m.poolId))))
        def removeMember(poolId: RidePoolId, rideId: RideId): Task[Boolean] = membersRef.modify { ms => val before = ms.size; val after = ms.filterNot(m => m.poolId == poolId && m.rideId == rideId); (before > after.size, after) }
        def updateMemberStatus(poolId: RidePoolId, rideId: RideId, status: PoolMemberStatus): Task[Boolean] = membersRef.modify { ms => var found = false; val updated = ms.map { m => if m.poolId == poolId && m.rideId == rideId then { found = true; m.copy(status = status) } else m }; (found, updated) }
    )
  ...
  private val inMemorySessionRepositoryLayer: ZLayer[Any, Nothing, SessionRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(List[Session](testSession)).map { store =>
        new SessionRepository:
          def create(s: Session): Task[Session] = store.update(_ :+ s).as(s)
          def findByUserId(userId: PersonId): Task[List[Session]] = store.get.map(_.filter(s => s.userId == userId && s.isActive))
          def findByToken(token: String): Task[Option[Session]] = store.get.map(_.find(s => s.token == token && s.isActive))
          def updateLastActive(sessionId: SessionId): Task[Unit] = store.update(_.map(s => if s.id == sessionId then s.copy(lastActiveAt = Instant.now()) else s)).unit
          def deactivate(sessionId: SessionId): Task[Boolean] = store.modify { ss => val updated = ss.map(s => if s.id == sessionId then s.copy(isActive = false) else s); (ss.exists(_.id == sessionId), updated) }
          def deactivateAllForUser(userId: PersonId): Task[Int] = store.modify { ss => val updated = ss.map(s => if s.userId == userId then s.copy(isActive = false) else s); (ss.count(s => s.userId == userId && s.isActive), updated) }
          def deactivateAllExcept(userId: PersonId, currentSessionId: SessionId): Task[Int] = store.modify { ss => val updated = ss.map(s => if s.userId == userId && s.id != currentSessionId then s.copy(isActive = false) else s); (ss.count(s => s.userId == userId && s.id != currentSessionId && s.isActive), updated) }
      }
    )
  ...
  private val inMemoryExpenseRepositoryLayer: ZLayer[Any, Nothing, ExpenseRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(Map[ExpenseId, Expense](testExpenseId -> testExpense)).map { store =>
        new ExpenseRepository:
          ...
          def findByRideId(rideId: RideId): Task[List[Expense]] = store.get.map(_.values.filter(_.rideId.contains(rideId)).toList)
          def findByDriverId(driverId: PersonId): Task[List[Expense]] = store.get.map(_.values.filter(_.driverId == driverId).toList)
          def findByCompanyId(companyId: CompanyId): Task[List[Expense]] = store.get.map(_.values.filter(_.companyId == companyId).toList)
          def update(e: Expense): Task[Expense] = store.update(_.updated(e.id, e)).as(e)
          def delete(id: ExpenseId): Task[Boolean] = store.modify { m => val existed = m.contains(id); (existed, m.removed(id)) }
          def sumByDriver(driverId: PersonId, companyId: CompanyId, from: Instant, to: Instant): Task[BigDecimal] =
            store.get.map(_.values.filter(e => e.driverId == driverId && e.companyId == companyId && !e.createdAt.isBefore(from) && e.createdAt.isBefore(to)).map(_.amount).sum)
      }
    )
  ...
  private val inMemoryRideTemplateRepositoryLayer: ZLayer[Any, Nothing, RideTemplateRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(Map[RideTemplateId, RideTemplate](testRideTemplateId -> testRideTemplate)).map { store =>
        new RideTemplateRepository:
          ...
          def findByCompanyId(companyId: CompanyId): Task[List[RideTemplate]] = store.get.map(_.values.filter(_.companyId == companyId).toList)
          def findActiveByCompanyId(companyId: CompanyId): Task[List[RideTemplate]] = store.get.map(_.values.filter(t => t.companyId == companyId && t.isActive).toList)
          def update(t: RideTemplate): Task[RideTemplate] = store.update(_.updated(t.id, t)).as(t)
          def delete(id: RideTemplateId): Task[Boolean] = store.modify { m => val existed = m.contains(id); (existed, m.removed(id)) }
          def deactivate(id: RideTemplateId): Task[Boolean] = store.modify { m => (m.contains(id), m.updatedWith(id)(_.map(_.copy(isActive = false)))) }
      }
    )
  ...
  private val inMemoryGeofenceRepositoryLayer: ZLayer[Any, Nothing, GeofenceRepository] =
    ZLayer.fromZIO(
      for {
        ...
        def findByCompanyId(companyId: CompanyId): Task[List[Geofence]] = geofencesRef.get.map(_.values.filter(_.companyId == companyId).toList)
        def findActiveByCompanyId(companyId: CompanyId): Task[List[Geofence]] = geofencesRef.get.map(_.values.filter(g => g.companyId == companyId && g.isActive).toList)
        ...
        def delete(id: GeofenceId): Task[Boolean] = geofencesRef.modify { m => val existed = m.contains(id); (existed, m.removed(id)) }
        def saveAlert(alert: GeofenceAlert): Task[GeofenceAlert] = alertsRef.update(_ :+ alert).as(alert)
        def findAlertsByCompany(companyId: CompanyId, limit: Int): Task[List[GeofenceAlert]] = alertsRef.get.map(_.filter(_.companyId == companyId).sortBy(_.timestamp)(using Ordering[java.time.Instant].reverse).take(limit))
        def findAlertsByDriver(driverId: PersonId, limit: Int): Task[List[GeofenceAlert]] = alertsRef.get.map(_.filter(_.driverId == driverId).sortBy(_.timestamp)(using Ordering[java.time.Instant].reverse).take(limit))
    )
  ...
  private val inMemoryBlacklistRepositoryLayer: ZLayer[Any, Nothing, BlacklistRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(List[BlacklistEntry](testBlacklistEntry)).map { store =>
        new BlacklistRepository:
          def create(entry: BlacklistEntry): Task[BlacklistEntry] = store.update(es => es.filterNot(e => e.clientId == entry.clientId && e.driverId == entry.driverId) :+ entry).as(entry)
          def findByCompanyId(companyId: CompanyId): Task[List[BlacklistEntry]] = store.get.map(_.filter(e => e.companyId == companyId && e.isActive))
          def findByClientId(clientId: PersonId): Task[List[BlacklistEntry]] = store.get.map(_.filter(e => e.clientId == clientId && e.isActive))
          def findByDriverId(driverId: PersonId): Task[List[BlacklistEntry]] = store.get.map(_.filter(e => e.driverId == driverId && e.isActive))
          def isBlacklisted(clientId: PersonId, driverId: PersonId): Task[Boolean] = store.get.map(_.exists(e => e.clientId == clientId && e.driverId == driverId && e.isActive))
          def deactivate(id: BlacklistEntryId): Task[Boolean] = store.modify { es => val idx = es.indexWhere(_.id == id); if idx >= 0 then (true, es.updated(idx, es(idx).copy(isActive = false))) else (false, es) }
          def delete(id: BlacklistEntryId): Task[Boolean] = store.modify { es => val before = es.size; val after = es.filterNot(_.id == id); (before > after.size, after) }
      }
    )
  ...
  private val inMemoryNotificationRepositoryLayer: ZLayer[Any, Nothing, NotificationRepository] =
    ZLayer.fromZIO(
      Ref.Synchronized.make(Map[AppNotificationId, AppNotification](testNotificationId -> testNotification)).map { store =>
        new NotificationRepository:
          ...
          def findByPersonId(personId: PersonId, limit: Int, offset: Int): Task[List[AppNotification]] =
            store.get.map(_.values.filter(_.personId == personId).toList.sortBy(_.createdAt)(using Ordering[java.time.Instant].reverse).drop(offset).take(limit))
          def markAsRead(id: AppNotificationId): Task[Boolean] = store.modify { m => m.get(id) match { case Some(n) => (true, m.updated(id, n.copy(isRead = true))); case None => (false, m) } }
          def markAllAsRead(personId: PersonId): Task[Unit] = store.update(_.map { case (k, n) => if n.personId == personId then (k, n.copy(isRead = true)) else (k, n) }).unit
          def countUnread(personId: PersonId): Task[Int] = store.get.map(_.values.count(n => n.personId == personId && !n.isRead))
          def delete(id: AppNotificationId): Task[Boolean] = store.modify { m => val existed = m.contains(id); (existed, m.removed(id)) }
          def deleteAllForPerson(personId: PersonId): Task[Unit] = store.update(_.filterNot((_, n) => n.personId == personId)).unit
      }
    )
    ...
    .provide(
      ...
      ZLayer.succeed[RideRatingRepository] {
        ...
        new RideRatingRepository:
          ...
          def findByRideId(rideId: RideId): Task[Option[RideRating]]     = ZIO.succeed(store.values.asScala.find(_.rideId == rideId))
          def findByDriverId(driverId: PersonId): Task[List[RideRating]] = ZIO.succeed(store.values.asScala.filter(_.driverId == driverId).toList)
          def getDriverAvgRating(driverId: PersonId): Task[Option[Double]] =
            ZIO.succeed { val rs = store.values.asScala.filter(_.driverId == driverId).map(_.rating).toList; if rs.isEmpty then None else Some(rs.sum.toDouble / rs.size) }
      },
      ...
      ZLayer.succeed[ClientCompanyRepository] {
        val seeded = ClientCompany(testBillingClientCompanyId, "Test BMW AG", testCompanyId1, Some("billing@bmw.de"), Some("+4989382-0"), Some("Petuelring 130, 80788 München"))
        new ClientCompanyRepository:
          private val store = new ConcurrentHashMap[ClientCompanyId, ClientCompany](Map(testBillingClientCompanyId -> seeded).asJava)
          def findById(id: ClientCompanyId): Task[Option[ClientCompany]]         = ZIO.succeed(Option(store.get(id)))
          def findByTaxiCompany(taxiCompanyId: CompanyId): Task[List[ClientCompany]] =
            ZIO.succeed(store.values.asScala.filter(_.taxiCompanyId == taxiCompanyId).toList)
          def create(company: ClientCompany): Task[ClientCompany]                = ZIO.succeed { store.put(company.id, company); company }
          def update(company: ClientCompany): Task[ClientCompany]                = ZIO.succeed { store.put(company.id, company); company }
          def delete(id: ClientCompanyId): Task[Boolean] =
            ZIO.succeed(Option(store.get(id)).isDefined && { store.remove(id); true })
      },
      // Billing
