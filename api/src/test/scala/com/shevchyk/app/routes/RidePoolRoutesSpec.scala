...
class StubRideService extends RideService:
  ...
  def createRide(request: CreateRideRequest): IO[RideError, Ride]                                                           = ZIO.fail(RideError.RideNotFound(RideId(UUID.randomUUID())))
  ...
  def cancelRideWithReason(rideId: RideId, userId: PersonId, userRole: PersonRole, request: CancelRideRequest): IO[RideError, Ride] = ZIO.fail(RideError.RideNotFound(rideId))
  def getCancellationStats(companyId: CompanyId): IO[RideError, Map[String, Int]]                                           = ZIO.succeed(Map.empty)
  def updateRideStatus(rideId: RideId, request: UpdateRideStatusRequest, userId: PersonId, userRole: PersonRole): IO[RideError, Ride] = ZIO.fail(RideError.RideNotFound(rideId))
...
object RidePoolRoutesSpec extends ZIOSpecDefault {
  ...
  private def makePool(companyId: UUID = taxiCompanyId, status: PoolStatus = PoolStatus.Open): RidePool =
    RidePool(
    ...
    suite("POST /api/pools")(
      test("dispatcher creates pool without rides") {
        for {
          ...
          request  = Request.post(URL.decode("/api/pools").toOption.get, Body.fromString(body))
      ...
      test("returns 403 for client role") {
        for {
          ...
          request  = Request.post(URL.decode("/api/pools").toOption.get, Body.fromString("""{"rideIds":[]}"""))
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
    ...
    suite("GET /api/pools")(
      test("dispatcher lists pools for own company") {
        for {
          ...
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode("/api/pools").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
          bodyStr <- resp.body.asString.orDie
    ...
    suite("GET /api/pools/open")(
      test("returns only open pools") {
        for {
          ...
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode("/api/pools/open").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
          bodyStr <- resp.body.asString.orDie
          list    <- ZIO.fromEither(bodyStr.fromJson[List[RidePool]]).mapError(new RuntimeException(_)).orDie
    ...
    suite("GET /api/pools/:id")(
      test("dispatcher gets pool details") {
        for {
          ...
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode(s"/api/pools/${pool.id.value}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
          bodyStr <- resp.body.asString.orDie
      ...
      test("returns error for unknown pool") {
        for {
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode(s"/api/pools/${UUID.randomUUID()}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
        } yield assertTrue(resp.status != Status.Ok)
      },
    ...
    suite("PUT /api/pools/:id/status")(
      test("dispatcher updates pool status") {
        for {
          ...
          request  = Request.put(URL.decode(s"/api/pools/${pool.id.value}/status").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
          bodyStr <- resp.body.asString.orDie
    ...
    suite("GET /api/pools/ride/:rideId")(
      test("returns pool when ride is in a pool") {
        for {
          ...
          token  <- generateToken(dispatcherId)
          resp   <- runRequest(
                      Request.get(URL.decode(s"/api/pools/ride/${rideId.value}").toOption.get)
                        .addHeader(Header.Authorization.Bearer(token))
                    )
        } yield assertTrue(resp.status == Status.Ok)
      },
      ...
      test("returns 404 when ride is not in any pool") {
        for {
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode(s"/api/pools/ride/${UUID.randomUUID()}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
    ...
    suite("DELETE /api/pools/:id/rides/:rideId")(
      test("dispatcher removes ride from pool") {
        for {
          ...
          token  <- generateToken(dispatcherId)
          resp   <- runRequest(
                      Request.delete(URL.decode(s"/api/pools/${pool.id.value}/rides/${rideId.value}").toOption.get)
                        .addHeader(Header.Authorization.Bearer(token))
                    )
      ...
      test("returns 401 without token") {
        for {
          resp <- runRequest(
                    Request.delete(URL.decode(s"/api/pools/${UUID.randomUUID()}/rides/${UUID.randomUUID()}").toOption.get)
                  )
        } yield assertTrue(resp.status == Status.Unauthorized)
      }
    )
