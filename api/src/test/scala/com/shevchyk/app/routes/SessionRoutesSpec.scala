...
class InMemoryTokenRepository extends TokenRepository:
  ...
  def deleteByUserId(userId: UUID): Task[Unit]                    = ZIO.succeed { tokens = tokens.filter { case (_, uid) => uid != userId } }
...
object SessionRoutesSpec extends ZIOSpecDefault {
  ...
  private def makeSession(uid: UUID = userId, token: String = "some-token"): Session =
    Session(
    ...
    suite("GET /api/sessions")(
      test("returns user's active sessions with isCurrent flag") {
        for {
          ...
          resp    <- runRequest(
                       Request.get(URL.decode("/api/sessions").toOption.get)
    ...
    suite("POST /api/sessions")(
      test("creates a new session") {
        for {
          ...
          request  = Request.post(URL.decode("/api/sessions").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
    ...
    suite("DELETE /api/sessions/:id")(
      test("deletes specific session") {
        for {
          ...
          _       <- repo.create(session)
          resp    <- runRequest(
                       Request.delete(URL.decode(s"/api/sessions/${session.id.value}").toOption.get)
                         .addHeader(Header.Authorization.Bearer(token))
                     )
    ...
    suite("DELETE /api/sessions")(
      test("revokes all other sessions except current") {
        for {
          ...
          resp     <- runRequest(
                        Request.delete(URL.decode("/api/sessions").toOption.get)
                          .addHeader(Header.Authorization.Bearer(token))
                      )
          bodyStr  <- resp.body.asString.orDie
          sessions <- repo.findByUserId(PersonId(userId))
