...
object AppRoutesSpec extends ZIOSpecDefault {
  ...
  private def run[R](routes: zio.http.Routes[R, Response])(request: Request): ZIO[R, Nothing, Response] = routes
    .run(request)
    .either
    .map {
  ...
  private val companySettingsSuite =
  ...
  private val notifPrefSuite =
    suite("NotificationPreferenceRoutes")(
      test("returns 401 without auth") {
        val request = Request.get(URL.decode("/api/notification-preferences").toOption.get)
        run(NotificationPreferenceRoutes.authenticatedRoutes)(request).map(r =>
          assertTrue(r.status == Status.Unauthorized)
        )
