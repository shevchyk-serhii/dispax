...
object GeofenceRoutesSpec extends ZIOSpecDefault {
  ...
  private def makeGeofence(companyId: UUID = taxiCompanyId, name: String = "Airport Zone"): Geofence = Geofence(
  ...
  def spec =
    suite("GeofenceRoutes")(
      suite("POST /api/geofences")(
        test("dispatcher creates geofence successfully") {
          for {
            token   <- generateToken(dispatcherId)
            body     =
              """{"name":"Service Area","geofenceType":"ServiceArea","centerLatitude":50.45,"centerLongitude":30.52,"radiusMeters":1000,"notifyOnEntry":true,"notifyOnExit":true}"""
            request  = Request
                         .post(URL.decode("/api/geofences").toOption.get, Body.fromString(body))
        ...
        test("returns error for invalid geofence type") {
          for {
            token  <- generateToken(dispatcherId)
            body    =
              """{"name":"Zone","geofenceType":"InvalidType","centerLatitude":50.0,"centerLongitude":30.0,"radiusMeters":100,"notifyOnEntry":true,"notifyOnExit":false}"""
            request = Request
                        .post(URL.decode("/api/geofences").toOption.get, Body.fromString(body))
                        .addHeader(Header.Authorization.Bearer(token))
            resp   <- runRequest(request)
        ...
        test("returns 401 without token") {
          for {
            body <- ZIO.succeed(
                      """{"name":"Zone","geofenceType":"Airport","centerLatitude":50.0,"centerLongitude":30.0,"radiusMeters":100,"notifyOnEntry":true,"notifyOnExit":false}"""
                    )
      ...
      suite("GET /api/geofences")(
        test("dispatcher gets list for own company") {
          for {
            ...
            resp    <- runRequest(
                         Request
                           .get(URL.decode("/api/geofences").toOption.get)
                           .addHeader(Header.Authorization.Bearer(token))
                       )
            bodyStr <- resp.body.asString.orDie
      ...
      suite("PUT /api/geofences/:id")(
        test("updates geofence for same company") {
          for {
            ...
            token   <- generateToken(dispatcherId)
            body     =
              s"""{"name":"Updated Zone","geofenceType":"Airport","centerLatitude":51.0,"centerLongitude":31.0,"radiusMeters":200,"notifyOnEntry":false,"notifyOnExit":true}"""
            request  = Request
                         .put(URL.decode(s"/api/geofences/${gf.id.value}").toOption.get, Body.fromString(body))
                         .addHeader(Header.Authorization.Bearer(token))
            resp    <- runRequest(request)
            bodyStr <- resp.body.asString.orDie
        ...
        test("returns error for geofence of different company") {
          for {
            ...
            token  <- generateToken(dispatcherId)
            body    =
              """{"name":"Hack","geofenceType":"Airport","centerLatitude":0.0,"centerLongitude":0.0,"radiusMeters":1,"notifyOnEntry":false,"notifyOnExit":false}"""
            request = Request
                        .put(URL.decode(s"/api/geofences/${gf.id.value}").toOption.get, Body.fromString(body))
                        .addHeader(Header.Authorization.Bearer(token))
            resp   <- runRequest(request)
      ...
      suite("DELETE /api/geofences/:id")(
        test("deletes geofence for same company") {
          for {
            ...
            resp  <- runRequest(
                       Request
                         .delete(URL.decode(s"/api/geofences/${gf.id.value}").toOption.get)
                         .addHeader(Header.Authorization.Bearer(token))
                     )
      ...
      suite("GET /api/geofences/alerts")(
        test("returns alerts for company") {
          for {
            ...
            resp    <- runRequest(
                         Request
                           .get(URL.decode("/api/geofences/alerts").toOption.get)
                           .addHeader(Header.Authorization.Bearer(token))
                       )
            bodyStr <- resp.body.asString.orDie
      ...
      suite("GET /api/geofences/alerts/driver/:driverId")(
        test("returns alerts for specific driver") {
          for {
            ...
            resp     <- runRequest(
                          Request
                            .get(URL.decode(s"/api/geofences/alerts/driver/$driverPid").toOption.get)
                            .addHeader(Header.Authorization.Bearer(token))
                        )
            bodyStr  <- resp.body.asString.orDie
            list     <- ZIO.fromEither(bodyStr.fromJson[List[GeofenceAlert]]).mapError(new RuntimeException(_)).orDie
