...
object BlacklistRoutesSpec extends ZIOSpecDefault {
  ...
  def spec = suite("BlacklistRoutes")(
    ...
    suite("GET /api/blacklist")(
      test("dispatcher lists entries for own company") {
        for {
          ...
          resp  <- runRequest(
                     Request.get(URL.decode("/api/blacklist").toOption.get)
    ...
    suite("POST /api/blacklist")(
      test("dispatcher creates blacklist entry") {
        for {
          ...
          request  = Request.post(URL.decode("/api/blacklist").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
    ...
    suite("GET /api/blacklist/check")(
      test("returns blacklisted true for existing pair") {
        for {
          ...
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.get(URL.decode(s"/api/blacklist/check?clientId=$clientUserId&driverId=$driverId").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
          bodyStr <- resp.body.asString.orDie
      ...
      test("returns blacklisted false for unknown pair") {
        for {
          ...
          resp  <- runRequest(
                     Request.get(
                       URL.decode(s"/api/blacklist/check?clientId=${UUID.randomUUID()}&driverId=${UUID.randomUUID()}").toOption.get
                     ).addHeader(Header.Authorization.Bearer(token))
                   )
          bodyStr <- resp.body.asString.orDie
    ...
    suite("DELETE /api/blacklist/:id")(
      test("dispatcher deactivates entry successfully") {
        for {
          ...
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.delete(URL.decode(s"/api/blacklist/${entry.id.value}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
      ...
      test("returns 404 for unknown entry") {
        for {
          token <- generateToken(dispatcherId)
          resp  <- runRequest(
                     Request.delete(URL.decode(s"/api/blacklist/${UUID.randomUUID()}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
