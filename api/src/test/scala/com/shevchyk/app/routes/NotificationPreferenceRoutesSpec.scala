...
object NotificationPreferenceRoutesSpec extends ZIOSpecDefault {
  ...
  def spec = suite("NotificationPreferenceRoutes")(
    ...
    suite("GET /api/notification-preferences")(
      test("returns default preferences when none saved") {
        for {
          token <- generateToken()
          resp  <- runRequest(
                     Request.get(URL.decode("/api/notification-preferences").toOption.get)
    ...
    suite("PUT /api/notification-preferences")(
      test("updates preferences for authenticated user") {
        for {
          ...
          request  = Request.put(URL.decode("/api/notification-preferences").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
