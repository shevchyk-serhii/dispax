...
class LocalInMemoryClientCompanyRepo extends ClientCompanyRepository:
  ...
  def findByTaxiCompany(taxiId: CompanyId): Task[List[ClientCompany]] = ZIO.succeed(store.values.asScala.filter(_.taxiCompanyId == taxiId).toList.sortBy(_.name))
...
class LocalInMemoryPersonRepo extends PersonRepository:
  ...
  def findByRoleAndCompany(role: PersonRole, companyId: CompanyId): Task[List[Person]]        = ZIO.succeed(store.values.asScala.filter(p => p.role == role && p.companyId.contains(companyId)).toList)
  def findByCompanyId(companyId: CompanyId): Task[List[Person]]                               = ZIO.succeed(store.values.asScala.filter(_.companyId.contains(companyId)).toList)
  ...
  def findByClientCompany(clientCompanyId: ClientCompanyId): Task[List[Person]]               = ZIO.succeed(store.values.asScala.filter(_.clientCompanyId.contains(clientCompanyId)).toList)
...
object ClientCompanyRoutesSpec extends ZIOSpecDefault {
  ...
  private def runRequest(req: Request): ZIO[ClientCompanyRepository & PersonRepository & JwtService, Nothing, Response] =
    ClientCompanyRoutes.authenticatedRoutes.run(req).either.map {
      case Left(r)  => r.merge
      case Right(r) => r
    }

  private def makeCompany(taxiId: UUID = taxiCompanyId, name: String = "Acme Corp"): ClientCompany =
    ClientCompany(
  ...
  def spec = suite("ClientCompanyRoutes")(
    ...
    suite("GET /api/client-companies")(
      test("dispatcher gets list for own taxi company") {
        for {
          ...
          request  = Request.get(URL.decode("/api/client-companies").toOption.get)
      ...
      test("returns 403 for client role") {
        for {
          ...
          resp  <- runRequest(
                     Request.get(URL.decode("/api/client-companies").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
                   )
    ...
    suite("POST /api/client-companies")(
      test("dispatcher creates company successfully") {
        for {
          ...
          request  = Request.post(URL.decode("/api/client-companies").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
          bodyStr <- resp.body.asString.orDie
    ...
    suite("GET /api/client-companies/:id")(
      test("returns company belonging to same taxi company") {
        for {
          ...
          token   <- generateToken(dispatcherId)
          request  = Request.get(URL.decode(s"/api/client-companies/${company.id.value}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
          bodyStr <- resp.body.asString.orDie
      ...
      test("returns 500 for unknown id") {
        for {
          token   <- generateToken(dispatcherId)
          request  = Request.get(URL.decode(s"/api/client-companies/${UUID.randomUUID()}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
    ...
    suite("PUT /api/client-companies/:id")(
      test("updates company name") {
        for {
          ...
          request  = Request.put(URL.decode(s"/api/client-companies/${company.id.value}").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
          bodyStr <- resp.body.asString.orDie
          result  <- ZIO.fromEither(bodyStr.fromJson[ClientCompany]).mapError(new RuntimeException(_)).orDie
    ...
    suite("DELETE /api/client-companies/:id")(
      test("deletes company with no members") {
        for {
          ...
          company <- repo.create(makeCompany())
          token   <- generateToken(dispatcherId)
          request  = Request.delete(URL.decode(s"/api/client-companies/${company.id.value}").toOption.get)
                       .addHeader(Header.Authorization.Bearer(token))
          resp    <- runRequest(request)
    ...
    suite("GET /api/client-companies/:id/members")(
      test("returns members of client company") {
        for {
          ...
          token      <- generateToken(dispatcherId)
          request     = Request.get(URL.decode(s"/api/client-companies/${company.id.value}/members").toOption.get)
                          .addHeader(Header.Authorization.Bearer(token))
          resp       <- runRequest(request)
          bodyStr    <- resp.body.asString.orDie
