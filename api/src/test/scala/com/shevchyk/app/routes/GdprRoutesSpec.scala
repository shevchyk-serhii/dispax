  ...
  def spec = suite("GdprRoutes")(
    ...
    suite("GET /api/gdpr/consents")(
      test("returns empty list when no consents") {
        for {
          token <- generateToken()
          resp  <- runRequest(
                     Request.get(URL.decode("/api/gdpr/consents").toOption.get)
      ...
      test("returns user's consents") {
        for {
          ...
          _     <- repo.createConsent(GdprConsent(
    ...
    suite("PUT /api/gdpr/consents")(
      test("grants a consent") {
        for {
          ...
          request  = Request.put(URL.decode("/api/gdpr/consents").toOption.get, Body.fromString(body))
                       .addHeader(Header.Authorization.Bearer(token))
    ...
    suite("GET /api/gdpr/export")(
      test("returns data export for user") {
        for {
          token   <- generateToken()
          resp      <- runRequest(
                         Request.get(URL.decode("/api/gdpr/export").toOption.get)
                           .addHeader(Header.Authorization.Bearer(token))
                       )
          bodyStr   <- resp.body.asString.orDie
    ...
    suite("POST /api/gdpr/deletion-request")(
      test("creates deletion request") {
        for {
          token   <- generateToken()
          resp    <- runRequest(
                       Request.post(URL.decode("/api/gdpr/deletion-request").toOption.get, Body.empty)
                         .addHeader(Header.Authorization.Bearer(token))
                     )
          bodyStr <- resp.body.asString.orDie
    ...
    suite("GET /api/gdpr/requests")(
      test("returns user's GDPR requests") {
        for {
          repo     <- ZIO.service[GdprRepository]
          _        <- repo.createRequest(GdprRequest(
