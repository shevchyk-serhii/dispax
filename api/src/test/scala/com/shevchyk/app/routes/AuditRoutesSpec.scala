...
object AuditRoutesSpec extends ZIOSpecDefault {
  ...
  private def makeEntry(companyId: UUID = taxiCompanyId, entityType: String = "ride"): AuditLogEntry = AuditLogEntry(
    ...
    suite("AuditRoutes")(
      suite("GET /api/audit")(
        ...
        test("returns 401 without token") {
          for {
            resp <- runRequest(
                      Request.get(URL.decode("/api/audit?entityType=ride&entityId=" + UUID.randomUUID()).toOption.get)
                    )
        ...
        test("returns 403 for client role") {
          for {
            ...
            resp  <- runRequest(
                       Request
                         .get(URL.decode(s"/api/audit?entityType=ride&entityId=${UUID.randomUUID()}").toOption.get)
                         .addHeader(Header.Authorization.Bearer(token))
                     )
        ...
        test("returns error when entityType param is missing") {
          for {
            ...
            resp  <- runRequest(
                       Request
                         .get(URL.decode(s"/api/audit?entityId=${UUID.randomUUID()}").toOption.get)
                         .addHeader(Header.Authorization.Bearer(token))
                     )
      ...
      suite("GET /api/audit/recent")(
        ...
        test("respects limit parameter") {
          for {
            ...
            token   <- generateToken(dispatcherId)
            request  = Request
                         .get(URL.decode("/api/audit/recent?limit=2").toOption.get)
                         .addHeader(Header.Authorization.Bearer(token))
            resp    <- runRequest(request)
            bodyStr <- resp.body.asString.orDie
            list    <- ZIO.fromEither(bodyStr.fromJson[List[AuditLogEntry]]).mapError(new RuntimeException(_)).orDie
