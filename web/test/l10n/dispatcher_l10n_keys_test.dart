...
void main() {
  ...
  group('dispatcher l10n keys — parity (all new keys non-empty)', () {
    test(
      'all new dispatcher keys are present and non-empty in English',
      () async {
  ...
  group('dispatcher l10n keys — semantic correctness', () {
    test(
      'homeTab is "Home"/"Startseite"/"Головна" (not reused "today")',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'billingTab is a distinct key from billingScreenTitle with proper localized values',
      () async {
    ...
    test(
      'moreTab and moreScreenTitle share the same value across locales',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'reassignAnyway vs assignAnyway are distinct (different verbs)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test('noAssignedRides vs noRidesCurrentlyAssigned are distinct', () async {
      ...
      expect(
        en.noRidesCurrentlyAssigned,
        equals('No rides currently assigned to drivers'),
      );
      expect(en.noAssignedRides, isNot(equals(en.noRidesCurrentlyAssigned)));
    });

    test(
      'driverMapMenuItem correct in all locales (used in nav + FilledButton)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'shared keys (cancel/retry/reassign) not duplicated in dispatcher batch',
      () async {
  ...
  group('dispatcher l10n keys — parameterized formatting', () {
    test(
      'dispatcherSubtitle interpolates weekday, date (String) and count (int)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'dispatcherSubtitle count is actually interpolated (not hardcoded)',
      () async {
        final en = await _l10n('en');
        expect(
    ...
    test('scheduleConflictsCount interpolates count (int)', () async {
      ...
      expect(
        de.scheduleConflictsCount(2),
        equals('Terminüberschneidungen (2)'),
      );
    ...
    test(
      'driverDelayedMessage interpolates driverName and slack (both String)',
      () async {
    ...
    test(
      'driverDelayedMessage driverName and slack are actually interpolated',
      () async {
        final en = await _l10n('en');
        expect(
    ...
    test(
      'ridesToReassignLabel interpolates selected and total (int)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'ridesReassignedMessage interpolates count (int) and driverName (String)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
  ...
  group('dispatcher l10n keys — shared key reuse (no duplicate synonyms)', () {
    test(
      'discardChangesTitle/stay/discard are shared and have unchanged values',
      () async {
    ...
    test(
      'assignAnywayTitle is the shared key for "Driver Busy" dialogs',
      () async {
    ...
    test(
      'billingScreenTitle is reused by billing AppBar (pre-existing shared key)',
      () async {
        final en = await _l10n('en');
        final de = await _l10n('de');
        final uk = await _l10n('uk');
    ...
    test(
      'noDriversAvailableForReassignment is shared between bulk_reassign and driver_schedule',
      () async {
